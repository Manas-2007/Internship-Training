...
import { useFocusEffect, useRouter } from "expo-router"; // 👈 Dono yahan se aayenge
...
export default function Wishlist() {
  ...
  const router = useRouter(); // Item par click karke details page par jaane ke liye
  ...
  // 🚀 MAGIC HOOK: Jab bhi ye screen focus mein aayegi, ye function chalega
  useFocusEffect(
    useCallback(() => {
      fetchWishlist();
    }, [])
  ...
  const removeItem = async (wishlistId: string, productId: string) => {
    try {
      ...
      fetchWishlist(); // Agar fail ho jaye toh wapas sync kar lo
    }
  };
      ...
      >
        {wishlistItems.length === 0 ? (
          <View className="flex-1 items-center justify-center">
            ...
            <Text className="text-neutral-800 text-xl font-bold mb-2">It feels so light!</Text>
          ...
          wishlistItems.map((item) => {
            ...
            return (
              <TouchableOpacity 
                key={item._id} 
                activeOpacity={0.9}
                onPress={() => router.push(`/product/${product._id}`)} // 👈 Clickable bana diya!
              ...
              >
                <View className="bg-neutral-100 rounded-xl overflow-hidden">
                  <Image source={{ uri: imageUrl }} className="w-24 h-32 object-cover" />
                </View>
                
                <View className="flex-1 ml-4 justify-center">
                  <Text className="text-neutral-500 text-xs font-extrabold mb-1 tracking-widest uppercase">{product.brand || "Brand"}</Text>
                  <Text className="text-neutral-800 text-sm font-semibold mb-2 leading-5" numberOfLines={2}>{product.name || "Product Name"}</Text>
                  
                  <View className="flex-row items-center">
                    <Text className="text-neutral-900 font-black text-lg mr-2">₹{product.price || 0}</Text>
                    {product.discount && (
                      <Text className="text-[#ff3f6c] text-xs font-bold bg-pink-50 px-2 py-1 rounded">{product.discount}</Text>
                    )}
                  </View>
                </View>
                ...
                <TouchableOpacity 
                  ...
                  onPress={(e) => {
                    e.stopPropagation(); // 👈 Taki card click trigger na ho jab delete dabayein
