      ...
      >
        ...
        <View className="w-full mt-2">
          ...
          <View className="absolute bottom-4 w-full flex-row justify-center gap-2">
            {bannerData.map((_, i) => (
              <View
                key={i}
                className={`h-2 rounded-full ${activeIndex === i ? "w-4 bg-white" : "w-2 bg-white/50"}`}
              />
            ))}
          </View>
        </View>
        ...
        <View className="mt-6 px-4">
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-lg font-black text-neutral-800 tracking-wide">
              SHOP BY CATEGORY
            </Text>
            <TouchableOpacity
        ...
        <View className="mt-6 py-5 bg-white px-4">
          <Text className="text-xl font-black text-neutral-800 tracking-wide mb-5">
            DEALS OF THE DAY
          </Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {deals?.map((deal: any, index: number) => (
              <TouchableOpacity
                key={deal._id || index}
                className="mr-5 w-72 h-44 rounded-2xl overflow-hidden relative shadow-lg shadow-neutral-300"
                activeOpacity={0.9}
                onPress={() => {
                  if (deal.productId) {
              ...
              >
                ...
                <View className="absolute inset-0 bg-black/30 p-5 justify-between">
                  ...
                  <View>
                    <Text className="text-white text-3sm font-black shadow-lg">
                      {deal.title}
                    </Text>
        ...
        <View className="mt-6 px-4 mb-24">
          <Text className="text-lg font-black text-neutral-800 tracking-wide mb-6">
            TRENDING NOW
          </Text>
          <View className="flex-row flex-wrap justify-between">
            {products?.map((product: any) => (
              ...
              >
                <View className="relative w-full h-60 bg-neutral-100">
                  ...
                  >
                    <Ionicons
                      ...
                      size={18}
                      color={
                        wishlistIds?.includes(product._id) ? "#ff3f6c" : "#000"
                      }
                    />
                  </TouchableOpacity>
                ...
                <View className="p-3">
                  <Text
                    className="text-neutral-900 text-[14px] font-extrabold"
                    numberOfLines={1}
                  >
                    {product.brand}
                  </Text>
                  <Text
                    className="text-neutral-500 text-[12px] mb-2"
                    numberOfLines={1}
                  >
                    {product.name}
                  </Text>
                  <View className="flex-row items-center">
                    <Text className="text-neutral-900 font-bold text-[15px]">
                      ₹{product.price}
                    </Text>
                  </View>
                </View>
