...
export default function ProductDetails() {
  ...
  const { products, wishlistIds, setWishlistIds, recordProductView } =
    useGlobalContext();
  ...
  return (
    ...
    >
      ...
      <View
        className="w-full max-w-[1400px] mx-auto flex-1 relative"
        style={{ backgroundColor: colors.background }}
      >
        <View
          className={`absolute z-20 ${isLargeScreen ? "top-6 left-6" : "top-4 left-4"}`}
        >
          <TouchableOpacity
            onPress={() => router.back()}
            activeOpacity={0.8}
        ...
        <ScrollView
          ...
          contentContainerStyle={{
            flexGrow: 1,
            paddingBottom: isLargeScreen ? 60 : insets.bottom + 120,
          }}
          bounces={false}
        >
          <View
            className={`w-full ${isLargeScreen ? "flex-row px-8 pt-8 gap-12" : "flex-col"}`}
          >
