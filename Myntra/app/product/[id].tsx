...
import { View, Text, Image, TouchableOpacity, ScrollView, Dimensions, StatusBar, Alert, ActivityIndicator } from "react-native";
...
export default function ProductDetails() {
  ...
  // Destructure wishlistIds & setWishlistIds to manage state globally
  ...
  // Syncs accurately with global context!
  ...
  // ✅ FIXED WISHLIST LOGIC FOR PRODUCT PAGE
  const handleWishlistToggle = async () => {
    try {
      ...
      if (isWishlisted) {
        // Remove from Global State instantly (Optimistic UI)
  ...
  return (
    <View className="flex-1 bg-white">
        ...
        <View className="relative w-full h-[400px] bg-neutral-100">
          ...
          <View className="absolute bottom-4 w-full flex-row justify-center gap-2">
            {productImages.map((_: any, i: number) => (
              <View key={i} className={`h-2 rounded-full ${activeImageIndex === i ? "w-5 bg-[#ff3f6c]" : "w-2 bg-white/70"}`} />
            ))}
          </View>
        </View>

        <View className="p-5">
          <Text className="text-2xl font-black text-neutral-900 tracking-tight">{product.brand}</Text>
          <Text className="text-base text-neutral-500 mt-1 font-medium">{product.name}</Text>
          
          <View className="flex-row items-center mt-4">
            <Text className="text-2xl font-bold text-neutral-900">₹{product.price}</Text>
            {product.discount && (
              <Text className="text-[#f43365] font-bold ml-3 bg-pink-50 px-2 py-1 rounded text-sm">{product.discount}</Text>
            )}
          </View>
          <Text className="text-xs text-neutral-400 mt-1 font-medium">inclusive of all taxes</Text>

          <View className="mt-6 border-t border-neutral-100 pt-5">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-base font-bold text-neutral-800">Select Size</Text>
              <Text className="text-[#f43365] font-bold text-sm tracking-wide">SIZE CHART</Text>
            </View>
            <View className="flex-row gap-3">
              {sizes.map((size) => {
                ...
                return (
                  ...
                  >
                    <Text className={`font-bold text-base ${isSelected ? "text-white" : "text-neutral-700"}`}>{size}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
          ...
          <View className="mt-8 mb-24 border-t border-neutral-100 pt-5">
            <Text className="text-base font-bold text-neutral-800 mb-2">Product Details</Text>
            <Text className="text-neutral-600 leading-6 text-sm font-medium">{product.description}</Text>
          </View>
        </View>
      </ScrollView>
      ...
      <View className="absolute bottom-0 w-full bg-white border-t border-neutral-100 p-4 flex-row justify-between items-center pb-8 shadow-2xl">
        ...
        {/* ❤️ WISHLIST BUTTON (Working globally now!) */}
        <TouchableOpacity 
        ...
        >
          {isAddingToBag ? (
            ...
            <>
              ...
              <Text className="text-white font-black text-base ml-2 tracking-widest">ADD TO BAG</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
