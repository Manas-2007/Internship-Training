  ...
  try {
    ...
    if (error.name === "VersionError") {
      return res.status(409).json({
        message:
          "Conflict: Cart was updated in another session. Please refresh.",
      });
    }
...
exports.updateQuantity = async (req, res) => {
  try {
    ...
    if (error.name === "VersionError") {
      return res.status(409).json({
        message:
          "Conflict: Quantity was changed by another device. Please refresh.",
      });
    }
  ...
  try {
    ...
    for (const item of activeItems) {
      ...
      if (item.quantity > stockLimit) {
        issues.push(
          `${product.name || "Product"} only has ${stockLimit} units left in stock.`,
        );
      }
