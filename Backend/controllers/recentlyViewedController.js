...
exports.getRecommendations = async (req, res) => {
  try {
    const userId = req.user._id || req.user.id;
    const [history, userWishlistItems] = await Promise.all([
      RecentlyViewed.findOne({ user: userId }).populate("items.product"),
      Wishlist.find({ userId: userId }).populate("productId"),
    ]);
    ...
    const excludeIds = [
      ...new Set([...viewedProductIds, ...wishlistedProductIds]),
    ];
