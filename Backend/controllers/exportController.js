...
exports.exportTransactionsCSV = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="transactions_history.csv"',
    );
...
exports.downloadReceiptPDF = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="receipt_${transaction.providerTransactionId}.pdf"`,
    );
    ...
    doc.text(
      `Invoice ID: INV-${Date.now()}-${transaction.providerTransactionId.slice(-6).toUpperCase()}`,
    );
    doc.moveDown();
    ...
    doc
      .fontSize(12)
      .font("Helvetica")
      .text(`Transaction Ref: ${transaction.providerTransactionId}`);
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text(`Total Amount Paid: INR ${transaction.amount}`, { align: "right" });

    doc.moveDown(4);
    doc
      .fontSize(10)
      .font("Helvetica-Oblique")
      .text("This is a computer-generated secure receipt.", {
        align: "center",
        color: "grey",
      });
